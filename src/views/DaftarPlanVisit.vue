    ...
    <ion-page>
        ...
        <ion-content>
            ...
            <div>
                <ion-card>
                    ...
                    <ion-card-content>
                        ...
                        <ion-input type="date" v-model="sampaiTanggal" name="sampaiTanggal"
                            id="sampaiTanggal"></ion-input>
                        <button type="button" class="w-full bg-blue-500 text-white px-4 py-2 rounded-lg mt-4"
                        ...
                        <div class="flex flex-col w-full h-full space-y-2">
                            ...
                            <div class="flex flex-row w-full justify-between space-x-2">
                                ...
                                <p class="flex-initial w-44 text-right">{{ new
                                    Date(visit.tanggal_plan).toLocaleDateString('id-ID') }}</p>
                            </div>
                            <div class="flex flex-row w-full justify-between space-x-2">
                                <label class="flex-initial w-56 font-semibold">Status Visit</label>
                                <ion-badge :color="visit.approval === 1 ? 'success' : 'warning'">{{ visit.approval === 1
                                    ? 'Sudah Visit' : 'Belum Visit' }}</ion-badge>
                            </div>
                            <div class="flex justify-end space-x-4 mt-2">

                                <ion-button expand="block" @click="setOpen(true, visit.callplan_id)">Edit</ion-button>
                                <ion-button color="danger"
                                    @click="deleteDataAlert(visit.callplan_id)">Delete</ion-button>
            ...
            <ion-infinite-scroll @ionInfinite="ionInfinite">
                <ion-infinite-scroll-content loading-text="Load more input daftar visit..."
            ...
            <ion-modal :is-open="isOpen">
                ...
                <ion-content class="ion-padding">
                    <div class="mb-4 mt-4">
                        ...
                        <ion-input type="date" v-model="tanggal" name="tanggal" id="tanggal"
                            class="form-input w-full px-4 py-2 border rounded-lg text-gray-700 focus:ring-red-500"></ion-input>
                    </div>

                    <div class="mb-4 mt-4">
                        <label for="toko" class="block text-gray-700 text-sm font-semibold mb-2">Toko *</label>
                        <select v-model="store_id" id="store_id" name="store_id"
                    ...
                    <button type="button" @click="updateCallPlanDetail(cp_id)"
                        class="mt-4 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"><ion-icon slot="start" :icon="checkmarkCircleSharp"></ion-icon> Save</button>
                </ion-content>
            </ion-modal>
...
import { IonButtons, IonButton, IonModal, IonHeader, IonToolbar, IonContent, IonTitle, alertController, IonRefresher, IonRefresherContent } from '@ionic/vue';
